...
def process_sq_upload(raw_df):
    """Full SQ pipeline: validate → aggregate → engineer features."""
    agg, stats = aggregate_to_daily(raw_df, SQ_DATE_COL, SQ_NUMERIC, 'Sinter Quality')
    sq_fe = engineer(agg, is_pp=False)
    return agg, sq_fe, stats


def process_pp_upload(raw_df):
    """Full PP pipeline: validate → aggregate → engineer features."""
    agg, stats = aggregate_to_daily(raw_df, PP_DATE_COL, PP_NUMERIC, 'Process Parameters')
...
def _render_sq_results(agg, fe, stats, existing_sq):
    ...
    existing_dates = set(pd.to_datetime(existing_sq[SQ_DATE_COL], errors='coerce').dropna())
    ...
    st.download_button('⬇️ Download Daily Aggregated CSV',
                       data=csv, file_name='sq_daily_aggregated.csv',
                       mime='text/csv', key='dl_sq')
...
def _render_pp_results(agg, fe, stats, existing_pp):
    ...
    existing_dates = set(pd.to_datetime(existing_pp[PP_DATE_COL], errors='coerce').dropna())
    ...
    csv = agg.to_csv(index=False)
    st.download_button('⬇️ Download Daily Aggregated CSV',
                       data=csv, file_name='pp_daily_aggregated.csv',
                       mime='text/csv', key='dl_pp')

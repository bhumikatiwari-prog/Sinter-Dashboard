...
def trend_chart(x, y, col, label, chart_type='trend'):
    ...
    BASE_LAYOUT = dict(
        ...
        margin       = dict(l=55, r=85, t=36, b=40),
        height       = 290,
        hovermode    = 'x unified',
    ...
    if chart_type == 'trend':
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='rgba(100,148,210,0.22)', width=1),
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='#f97316', width=3.0),
            name='7d Moving Avg',
        ...
        if len(anom_x) > 0:
            fig.add_trace(go.Scatter(
                ...
                name=f'Anomaly ({len(anom_x)})',
                hovertemplate='⚠ ANOMALY<br>%{x|%d %b %Y}<br>Value: %{y:.3f}<extra></extra>',
            ))
...
def render(pp_filt, pp_date, d_from, d_to):
    ...
    <div style="display:flex;gap:18px;flex-wrap:wrap;background:#0a1628;
      ...
      <span style="color:rgba(100,148,210,0.7)">━ Daily (raw)</span>
    ...
    for i in range(0, len(sel), 2):
        ...
        for j, label in enumerate(pair):
            ...
            if col_name not in pp_filt.columns:
                continue
            ...
            if len(d2) < 3:
                continue
            s = d2[col_name]
            stats  = param_stats(s, col_name)
            health, badge = param_health(s, col_name)
            with col_pair[j]:
                st.markdown(stat_card_html(label, col_name, stats, health, badge),
                            unsafe_allow_html=True)

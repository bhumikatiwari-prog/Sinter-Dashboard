...
def gauge(val, target, direction, title, color):
    ...
    if direction == '>=': lo,hi,ref=max(val,target)*0.9,max(val,target)*1.08,target
    else: lo,hi,ref=min(val,target)*0.92,max(val,target)*1.05,target
    fig = go.Figure(go.Indicator(
        ...
        number={'font':{'size':32,'color':'#e2e8f0','family':'JetBrains Mono'},'valueformat':'.2f'},
        title={'text':title,'font':{'size':13,'color':'#94b4d4'}},
        gauge={
            'axis':{'range':[lo,hi],'tickwidth':1,'tickcolor':'#1e3a5f','tickfont':{'size':9}},
            'bar':{'color':bar_color,'thickness':0.65},
            'bgcolor':'#0d1f3c','borderwidth':1,'bordercolor':'#1e3a5f',
            'steps':[{'range':[lo,ref],'color':'#0f2347'},{'range':[ref,hi],'color':'#0a1628'}],
            'threshold':{'line':{'color':'#fbbf24','width':2},'thickness':0.8,'value':ref},
        }
    ))
    fig.update_layout(paper_bgcolor='#0d1f3c',font=dict(color='#94b4d4'),
    ...
    for tab,target in zip([tab1,tab2,tab3],TARGETS):
        with tab:
            ...
            fig.add_trace(go.Scatter(x=d2[date_col],y=roll,mode='lines',
                name='7d Avg',line=dict(color=TC[target],width=2.5)))
            ...
            fig.update_layout(**clayout(title=f'{target} Trend with 7-day Rolling Average',height=320))
            st.plotly_chart(fig, use_container_width=True)

    st.markdown('<div class="section-header">🔔 QUALITY ALERTS</div>', unsafe_allow_html=True)
    a1,a2,a3 = st.columns(3)
    for col,target,direction in zip([a1,a2,a3],TARGETS,['>=','<=','>=']):
        recent = df[target].dropna().tail(7).mean()
        tgt = BF_TARGETS[target]
        ok = recent>=tgt if direction=='>=' else recent<=tgt
        badge = '<span class="status-ok">✅ ON TARGET</span>' if ok else '<span class="status-bad">⚠️ OFF TARGET</span>'
        col.markdown(f'<div class="insight-box"><h4>{target} — Last 7d Average</h4><p>Value: <b>{recent:.3f}</b> | Target: {"≥" if direction==">=" else "≤"}{tgt} {badge}</p></div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="section-header">⚙️ PROCESS PARAMETER MONITORING</div>', unsafe_allow_html=True)
    PROC_COLS = {
        'Machine speed':'Machine speed','WGF Speed':'WGF Speed',
        'ESP Inlet Temp.':'ESP Inlet Temp.','ESP Inlet Press.':'ESP Inlet Press.',
        'Feed':'Feed','Moisture':'Moisture','BTP Temperature':'BTP Temperature',
        'Cooler Speed':'Cooler Speed','Production':'Production',
        'Basicity':'Basicity(B2)','FeO':'%FeO','MgO':'%MgO','CaO':'%CaO',
        'SiO₂':'%SiO2','Al₂O₃':'%Al2O3'
    }
    pp_date = 'Date'
    if pp_date in pp_raw.columns:
        pp_raw[pp_date] = pd.to_datetime(pp_raw[pp_date], errors='coerce')
        pp_mask = (pp_raw[pp_date] >= d_from) & (pp_raw[pp_date] <= d_to)
        pp_filt = pp_raw[pp_mask].copy()
    else: pp_filt = pp_raw.copy()

    available = {k:v for k,v in PROC_COLS.items() if v in pp_filt.columns}
    sel = st.multiselect('Select Parameters to Plot', list(available.keys()),
                          default=list(available.keys())[:6])
    if sel:
        for i in range(0,len(sel),2):
            cols_row = st.columns(2)
            for j,s in enumerate(sel[i:i+2]):
                col_name = available[s]
                d2 = pp_filt[[pp_date,col_name]].dropna() if pp_date in pp_filt.columns else pp_filt[[col_name]].dropna()
                if len(d2)<2: continue
                with cols_row[j]:
                    fig = go.Figure()
                    x = d2[pp_date] if pp_date in d2.columns else d2.index
                    fig.add_trace(go.Scatter(x=x,y=d2[col_name],mode='lines+markers',
                        line=dict(color='#3b82f6',width=1.5),
                        marker=dict(size=3,color='#60a5fa'),name=s))
                    roll = d2[col_name].rolling(7,min_periods=1).mean()
                    fig.add_trace(go.Scatter(x=x,y=roll,mode='lines',
                        line=dict(color='#fbbf24',width=1.5,dash='dash'),name='7d Avg'))
                    fig.update_layout(**clayout(title=s,height=260))
                    st.plotly_chart(fig,use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with tabs[2]:
        ...
        for i,feat in enumerate(feats[::-1]):
            ...
            norm = (feat_vals - feat_vals.min())/(feat_vals.ptp()+1e-9)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if 'opt_result' in st.session_state:
        ...
        st.dataframe(rec_df.style.applymap(lambda v: 'color:#22c55e' if isinstance(v,str) and '▲' in v
